    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            f'viewBox="0 0 1200 800" role="img" data-theme="{theme}">'
        ),
        "<title>Divyanshu Goyal — AI architect, founder, and builder</title>",
        (
            "<desc>Operator console featuring an ASCII portrait, current AI and founder "
            "work, selected systems, and anonymous aggregate GitHub telemetry.</desc>"
        ),
        "<style>",
        (
            "text{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"
        ...
        (
            f'<rect x="18" y="18" width="1164" height="764" rx="14" '
            f'fill="{palette.panel}" stroke="{palette.border}"/>'
        ),
        f'<g fill="{palette.muted}">',
        _text(48, 56, "DEEVYANSHOO // OPERATOR", "eyebrow"),
        "</g>",
        f'<circle cx="1078" cy="51" r="5" fill="{palette.signal}"/>',
        f'<g fill="{palette.signal}">',
        _text(1092, 56, status_label, "eyebrow"),
        "</g>",
        f'<line x1="48" y1="78" x2="1152" y2="78" stroke="{palette.border}"/>',
        f'<g fill="{palette.accent}">',
        _text(48, 112, "PORTRAIT / 01", "section"),
        "</g>",
    ...
    for index, row in enumerate(PORTRAIT):
        lines.append(_text(48, portrait_y + index * 16, row, "portrait"))
    ...
    lines.extend(
        [
            ...
            f'<g fill="{palette.accent}">',
            _text(526, 112, "IDENTITY / MISSION", "section"),
            "</g>",
            f'<g fill="{palette.primary}">',
            _text(526, 154, NAME, "title"),
            "</g>",
            f'<g fill="{palette.accent}">',
            _text(526, 184, ROLE, "role"),
            "</g>",
            f'<g fill="{palette.secondary}">',
            _text(526, 218, POSITIONING, "body"),
            "</g>",
            f'<g fill="{palette.primary}">',
            _text(526, 252, TAGLINE, "tagline"),
            "</g>",
            f'<g fill="{palette.muted}">',
            _text(526, 278, "AI Architect @ ZS  ·  UPenn MSE Data Science", "small"),
            _text(526, 300, LOCATION, "small"),
            "</g>",
            f'<line x1="526" y1="326" x2="1152" y2="326" stroke="{palette.border}"/>',
            f'<g fill="{palette.accent}">',
            _text(526, 354, "CURRENT", "section"),
            "</g>",
            f'<g fill="{palette.primary}">',
            _text(526, 386, CURRENT_MISSION, "body"),
            "</g>",
            f'<g fill="{palette.secondary}">',
            _text(526, 412, SECONDARY_MISSION, "body"),
            "</g>",
            f'<g fill="{palette.accent}">',
            _text(526, 456, "SELECTED BUILDS", "section"),
            "</g>",
        ]
    )
    ...
    for build in SELECTED_BUILDS:
        lines.extend(
            [
                f'<g fill="{palette.primary}">',
                _text(526, build_y, build.label, "body"),
                "</g>",
                f'<g fill="{palette.muted}">',
                _text(650, build_y, build.detail, "small"),
                "</g>",
            ]
        )
        build_y += 26
    ...
    lines.extend(
        [
            ...
            f'<g fill="{palette.accent}">',
            _text(48, 596, "GITHUB / LIVE AGGREGATES", "section"),
            "</g>",
            f'<g fill="{palette.muted}">',
        ]
    )
    metrics = (
        ...
        (804, 630, "REPOS TOUCHED", f"{stats.repositories_contributed_to:,}"),
        (48, 676, "STARS EARNED", f"{stats.stars_earned:,}"),
        (
            300,
            676,
            "PRIVATE ACTIVITY",
            "AGGREGATED" if data.private_activity_aggregated else "PUBLIC ONLY",
        ),
    )
